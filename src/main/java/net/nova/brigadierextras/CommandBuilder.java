...
public class CommandBuilder {
                ...
                if (method.getParameters().length == 1) {
                    ...
                    dispatcher.register(
                            (LiteralArgumentBuilder<S>) LiteralArgumentBuilder.literal(name)
                                    .executes(ctx -> {
                                        try {
                                            Object obj = method.invoke(command, ctx.getSource());
    ...
    public static <S, T extends CommandSender<S>> void registerCommand(CommandDispatcher<S> dispatcher, Class<T> dispatcherClass, Function<S, T> conv, Object command) throws InvalidCommandException {
        Class<?> clazz = command.getClass();
